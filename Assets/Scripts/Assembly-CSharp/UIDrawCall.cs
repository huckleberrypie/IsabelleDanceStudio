...
public class UIDrawCall : MonoBehaviour
{
	...
	public int depth
	{
		...
		set
		{
			...
			{
				mDepth = value;
				if (mMat != null && material != null)
				{
					mMat.renderQueue = material.renderQueue + value;
				}
			}
		}
	}
	...
	private Mesh GetMesh(ref bool rebuildIndices, int vertexCount)
	{
		...
		if (mEven)
		{
			if (mMesh0 == null)
			{
				...
				mMesh0.name = "Mesh0 for " + material.name;
				mMesh0.MarkDynamic();
				rebuildIndices = true;
			}
		...
		if (mMesh1 == null)
		{
			...
			mMesh1.name = "Mesh1 for " + material.name;
			mMesh1.MarkDynamic();
			rebuildIndices = true;
		}
	...
	private void UpdateMaterials()
	{
		...
		if (mMat == null)
		{
			...
			mMat.CopyPropertiesFromMaterial(material);
			mMat.renderQueue = material.renderQueue + mDepth;
		}
		if (flag && mClipping != 0)
		{
			Shader shader = null;
		...
		if (mUseDepth)
		{
			...
			mDepthMat.mainTexture = material.mainTexture;
		}
	...
	public void Set(BetterList<Vector3> verts, BetterList<Vector3> norms, BetterList<Vector4> tans, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if (size > 0 && size == uvs.size && size == cols.size && size % 4 == 0)
		{
			...
			else if (mMat != null && mMat.mainTexture != material.mainTexture)
			{
				UpdateMaterials();
			}
			if (verts.size < 65000)
