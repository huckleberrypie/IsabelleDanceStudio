...
public class UILabel : UIWidget
{
		...
		set
		{
			if (mFont != value)
			{
				if (mFont != null && mFont.dynamicFont != null)
				{
					...
					dynamicFont.textureRebuildCallback = (Font.FontTextureRebuildCallback)Delegate.Remove(dynamicFont.textureRebuildCallback, new Font.FontTextureRebuildCallback(MarkAsChanged));
				}
				RemoveFromPanel();
				mFont = value;
				hasChanged = true;
				if (mFont != null && mFont.dynamicFont != null)
				{
					Font dynamicFont2 = mFont.dynamicFont;
					dynamicFont2.textureRebuildCallback = (Font.FontTextureRebuildCallback)Delegate.Combine(dynamicFont2.textureRebuildCallback, new Font.FontTextureRebuildCallback(MarkAsChanged));
					mFont.Request(mText);
				}
				MarkAsChanged();
			}
	...
	protected override void OnEnable()
	{
		...
		{
			Font dynamicFont = mFont.dynamicFont;
			dynamicFont.textureRebuildCallback = (Font.FontTextureRebuildCallback)Delegate.Combine(dynamicFont.textureRebuildCallback, new Font.FontTextureRebuildCallback(MarkAsChanged));
		}
		base.OnEnable();
	}
